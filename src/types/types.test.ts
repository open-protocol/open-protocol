...
test('transaction encode test', () => {
  const txJson = "{\"from\": \"7e9cd855ddb203964649da096ebba0515070db91a0bfcba96e4f692ad582f2dc\",\"to\": \"48cddaa3e83ba437487defec48e92f5023cffd67f2c7e506dace3977c662cc56\",\"value\": \"ff\",\"nonce\": 1,\"input\": \"00\"}"
  const unsignedTx = UnsignedTransaction.fromJson(txJson)

  expect(unsignedTx.toBuffer().toString('hex')).toBe('0220007e9cd855ddb203964649da096ebba0515070db91a0bfcba96e4f692ad582f2dc02200048cddaa3e83ba437487defec48e92f5023cffd67f2c7e506dace3977c662cc56020100ff0101000102010000')
})
...
test('transaction verify test', async () => {
  ...
  const signedTx = new SignedTransaction(unsignedTx.from, unsignedTx.to, unsignedTx.value, unsignedTx.nonce, unsignedTx.input, signature.toString('hex'))
...
test('account new test', async () => {
  ...
  expect(account.toBuffer().toString('hex')).toBe('0220009c02a283a8562478ccaf221b9ba2b7cf6348d1dc31a52c0a7af513ed88d62321020100000102002710020100000400000000')
})
