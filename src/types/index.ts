...
export class UnsignedTransaction implements IUnsignedTransaction {
  ...
  public static fromBuffer = (buffer: Buffer): UnsignedTransaction => {
    ...
    return new UnsignedTransaction(values[0], values[1], values[2], values[3], values[4])
  }
  ...
  public toBuffer = (): Buffer => {
    return Codec.encode([this.from, this.to, this.value, this.nonce, this.input])
  }
  ...
  public static fromSignedTx = (signedTx: SignedTransaction): UnsignedTransaction => {
    return new UnsignedTransaction(signedTx.from, signedTx.to, signedTx.value, signedTx.nonce, signedTx.input)
  }
}
...
export class SignedTransaction implements ISignedTransaction {
  ...
  public static fromBuffer = (buffer: Buffer): SignedTransaction => {
    ...
    return new SignedTransaction(values[0], values[1], values[2], values[3], values[4], values[5])
  }
  ...
  public toBuffer = (): Buffer => {
    return Codec.encode([this.from, this.to, this.value, this.nonce, this.input, this.signature])
  }
  ...
  public verify = async (): Promise<boolean> => {
    const unsignedTx = UnsignedTransaction.fromSignedTx(this)
    return await Keypair.verify(Codec.encodeString(this.signature), unsignedTx.toBuffer(), Codec.encodeString(this.from))
  }
}
...
export class Account implements IAccount {
  ...
  public static new = (publicKey: string): Account => {
    ...
    return new Account(publicKey, '00', 0, '00', new Map<MemoryKey, MemoryValue>())
  }
  ...
  public toBuffer = (): Buffer => {
    return Codec.encode([this.publicKey, this.balance, this.nonce, this.code, this.memory])
  }
}
...
export class BlockHeader implements IBlockHeader {
  ...
  public static new = (number: number, previous: string, txroot: string, stateroot: string): BlockHeader => {
    const timestamp = Date.now()
    const buffer = Codec.encode([number, previous, txroot, stateroot, timestamp])
  ...
  public toBuffer = (): Buffer => {
    return Codec.encode([this.number, this.previous, this.txroot, this.stateroot, this.timestamp])
  }
}
