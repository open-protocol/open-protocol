...
export class NetworkTask implements ITask {
  ...
  init = async (manager: TaskManager): Promise<void> => {
    ...
    const privateKey = Uint8Array.from(Codec.encodeString(process.env.PEER_PRIVATE))
    const publicKey = Uint8Array.from(Codec.encodeString(process.env.PEER_PUBLIC))
