...
import { Account, WasmCall } from '../types'
    ...
    const module = await WebAssembly.instantiate(contractCode, {
      env: {
        ...
        get: (offset: number, len: number): number => {
          ...
          const key = Codec.decode(Buffer.from(buffer, offset, len))[0] as MemoryKey
          const value = contractMemory.get(key)
          const valueBuffer = Codec.encode([value])
          new Uint8Array(buffer).subarray(len + valueBuffer.length).set(valueBuffer)
          return valueBuffer.length
        },
        put: (keyOffset: number, keyLen: number, valOffset: number, valLen: number) => {
          const buffer = (module.instance.exports.memory as WebAssembly.Memory).buffer
          const key = Codec.decode(Buffer.from(buffer, keyOffset, keyLen))[0] as MemoryKey
          const value = Codec.decode(Buffer.from(buffer, valOffset, valLen))[0] as MemoryValue
